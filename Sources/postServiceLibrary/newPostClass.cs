...
namespace postServiceLibrary
{
	class newPostClass
	{
		...
		// last_update_time ???
		public string posts_update(string session_token, string group_id, string post_id, string attachment_id_array, string last_update_time, string type, string event_type, string favorite)
		{
			...
			try
			{
				...
				_responseMessage = _r;
				//  MR_posts_update _ret = JsonConvert.DeserializeObject<MR_posts_update>(_r);
			...
			catch (WebException _e)
			{
				if (_e.Status == WebExceptionStatus.ProtocolError)
				{
					...
					List<_Error> ErrorCode = new List<_Error>{
						new _Error{ProtocolError= _res.StatusCode.ToString(), Description = _res.StatusDescription},
					 };

					var json = JsonConvert.SerializeObject(ErrorCode);
			...
			{
				_re = null;
				log.Error("Update Post, failed: " + _e.Data);
			}
			return _re;
		}
