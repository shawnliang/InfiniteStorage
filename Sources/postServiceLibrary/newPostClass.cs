...
namespace postServiceLibrary
{
	internal class newPostClass
	{
		...
		// last_update_time ???
		public void posts_update(string session_token, string group_id, string post_id, string attachment_id_array, string type, string event_type, string favorite,
		                         DateTime last_update_time, string email_list, string title, string sender)
		{
			log.Info("start Post_update");
			//string _re = null;

			string _post_id = post_id;
			_post_id = HttpUtility.UrlEncode(_post_id);
			...
			event_type = HttpUtility.UrlEncode(event_type);
			favorite = HttpUtility.UrlEncode(favorite);
		                        ...
		                        string preview, string type, string coverAttach, string share_email_list, string event_type, string favorite, string post_id, string title)
		{
			...
			share_email_list = HttpUtility.UrlEncode(share_email_list);
			event_type = HttpUtility.UrlEncode(event_type);
			favorite = HttpUtility.UrlEncode(favorite);
			title = HttpUtility.UrlEncode(title);
