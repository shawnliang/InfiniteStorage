...
namespace Wpf_testHTTP
{
	public partial class MainWindow : Window
	{
		...
		public void setRun_button()
		{
			...
			if (File.Exists(iniPath))
			{
				...
				if (RefreshKey_real != "")
				{
					...
					if (result) // exchange refresh token with refresh token
					{
		...
		public bool sendEmailList()
		{
			...
			string rel = labelCommand.inviteShared(label_id, sender_name, sender_msg, recipients_json);
		...
		private static string RefreshKey_saved;
		...
		/*
		private string user = "isserverId0.anonymous@waveface.com"; // "ruddytest29@gmail.com";
		private string password = "anonymous123456"; // "a+123456";
		//newPostClass _ws = new newPostClass();
		private string group_id = "";
		private string session_token_key = "";
		private List<string> object_arr = new List<string>(); // Attachments object_id 
		*/

		private bool uploadFinished;
		private string filename = @"C:/Users/" + machine_user + "/Pictures/2.jpg~C:/Users/" + machine_user + "/Pictures/video.mp4~C:/Users/" + machine_user + "/Pictures/3.jpg";
		private string email = "";
		private int no_of_attachments;
		private string[] arr;

		private List<string> email_arr = new List<string>(); // shared email address

		// save google api return contacts in gmail_arr & title_arr
		private List<string> gmail_arr = new List<string>(); // contacts data email
		private List<string> title_arr = new List<string>(); //  contacts data title
		// share
		private bool complete;
		private bool shareButtonClick;
		...
		private bool already_start;

		private void dispatcherTimer1_Tick(object sender, EventArgs e)
		{
			if (already_start) return;
			already_start = true;
			if (complete && shareButtonClick)
			{
				service_run();
				complete = false;
				shareButtonClick = false;
		...
		private const string clientID = "598605053299.apps.googleusercontent.com"; // company Google App ID
		...
		private List<string> scope2 = new List<string>
			                              {
				                              GoogleScope.ImapAndSmtp.Name,
				                              GoogleScope.EmailAddressScope.Name,
		...
		*/
		...
		// private HashSet<string> _set1 = new HashSet<string> { GoogleScope.ImapAndSmtp.Name };
		private NativeApplicationClient consumer;
		...
		private void wb1_LoadCompleted(object sender, NavigationEventArgs e)
		{
			...
			i0 = _str.IndexOf("Success code=", i0 + 1, StringComparison.Ordinal);

			if (i0 == -1)
			{
				i0 = _str.IndexOf("Denied error", i0 + 1, StringComparison.Ordinal);
				if (i0 >= 0)
				{
					myTabControl.SelectedIndex = 0;
					connectedPanel.Visibility = Visibility.Collapsed;
			...
			int i1 = _str.IndexOf("</title>", i0 + 1, StringComparison.Ordinal);
		...
		//private IAuthorizationState grantedAccess1;
		...
		private bool getAccessToken(string authCode)
		{
			...
			try
			{
				...
				foreach (XmlNode contact in contacts.GetElementsByTagName("entry"))
				{
					...
					XmlNode _email = contact.SelectSingleNode("gd:email", nsmgr);

					// Console.WriteLine("{0}: {1}",title.InnerText, email.Attributes["address"].Value);
					if (_email != null)
					{
						title_arr.Add(title.InnerText);
						gmail_arr.Add(_email.Attributes["address"].Value);
						emailCount++;
					}
				}
				getSuccess = true;
