...
import com.waveface.jmdns.impl.DNSIncoming;
import com.waveface.jmdns.impl.DNSOutgoing;
import com.waveface.jmdns.impl.DNSQuestion;
...
public class Responder extends DNSTask {
    ...
    public Responder(JMDNSImplement jMDNSImplement, DNSIncoming in, int port) {
    ...
    public void start(Timer timer) {
        ...
        int delay = (iAmTheOnlyOne && !_in.isTruncated()) ? 0 : DNSConstants.RESPONSE_MIN_WAIT_INTERVAL + JMDNSImplement.getRandom().nextInt(DNSConstants.RESPONSE_MAX_WAIT_INTERVAL - DNSConstants.RESPONSE_MIN_WAIT_INTERVAL + 1) - _in.elapseSinceArrival();
        if (delay < 0) {
            delay = 0;
        }
        if (logger.isLoggable(Level.FINEST)) {
